...
@Service
public class AbandonInstanceService implements AbandonInstanceUseCase {
    ...
    @Override
    @Transactional
    public ProcessInstance execute(InstanceId instance, String reason) {
        ...
        ProcessInstance abandoned = found.abandonedWith(reason, now);
        saveInstancePort.abandon(abandoned);
        appendProcessEventPort.append(ProcessEvent.onInstance(instance, ProcessAction.INSTANCE_ABANDONED, caller, now));
        return abandoned;
    }
}
