...
@Service
public class ProcessCategoryService implements ProcessCategoryUseCase {
    private final ProcessCategoryStorePort store;
    private final WorkspacePort workspace;
    private final Clock clock;

    public ProcessCategoryService(ProcessCategoryStorePort store, WorkspacePort workspace, Clock clock) {
        this.store = store;
    ...
    public List<Category> all() {
        return store.categoriesIn(workspace.currentWorkspaceId()).stream()
                .map(stored -> new Category(stored.id(), stored.name(), stored.runCount()))
                .toList();
    }
    ...
    @Override
    @Transactional
    @PreAuthorize("hasAuthority('PROCESS_EDIT_INSTANCE')")
    public void fileRun(UUID instanceId, Optional<UUID> categoryId) {
        UUID here = workspace.currentWorkspaceId();

        categoryId.ifPresent(id -> mustBeMine(id, here));

        store.fileRun(instanceId, here, categoryId.orElse(null));
    }
